...
function stringifyBytesLike(data) {
    if (Array.isArray(data))
        return ethers.hexlify(new Uint8Array(data));
...
function signCall(call, signer, overrides) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (signature !== undefined)
            return signature;
        signature = ethers.getBytes('_signTypedData' in signer
...
function upgradeDomain(domain) {
    const x = Object.assign(Object.assign({}, domain), { chainId: domain.chainId ? toBeHex(domain.chainId) : undefined });
    if (domain.salt) {
...
function upgradeCall(call) {
    const big2int = (b) => {
        ...
        if (b instanceof BigInt)
            return toBeHex(b.toString());
        return ethers.toQuantity(b);
    };
    return Object.assign(Object.assign({}, call), { gasPrice: big2int(call.gasPrice), value: big2int(call.value), data: call.data ? ethers.getBytes(call.data) : undefined });
}
